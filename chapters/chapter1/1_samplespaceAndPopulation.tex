...
For example, $c \notin C$ is read "the element c is not a member of (not in) te set C".
...
Here an \textbf{experiment} is the process that generates observations.
...
The following three axioms (knowledge we assume true without proof) are called \textbf{Kolmorogov's axioms} (\label{Kaxioms})
\begin{enumerate}
